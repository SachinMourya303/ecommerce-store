...
const Adminpage = () => {
  ...
  const fetchusers = async () => {
    try {
      const response = await axios.get(`http://localhost:5000/adminAuth/alladmin?name=${searchValue}`);
  ...
  const removeusers = async (mongoid) => {
    try {
      const response = await axios.delete('http://localhost:5000/adminAuth/deleteadmin', {
        params: {
          id: mongoid
        }
      })
