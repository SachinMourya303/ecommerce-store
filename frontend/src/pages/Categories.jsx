...
const Shop = ({ user, setFavProducts, setCartProducts, setfetchedFavProducts, setfetchedCartProducts }) => {
  ...
  const fetchProducts = async () => {
    try {
      const response = await axios.get(`http://localhost:5000/products/getallproducts?category=${category}`);
  ...
  return (
    <div>
      ...
      <div className='flex flex-col items-center w-full md:pt-15 transition-all'>
        <div className='flex my-5 md:mt-10 text-gray-700 w-full justify-start px-6 md:px-16 lg:px-24 xl:px-32'><NavLink to='/'>Home</NavLink> <ChevronRight /> <NavLink to='/'>Shop</NavLink> </div>
      </div>
      <AnimatePresence>
